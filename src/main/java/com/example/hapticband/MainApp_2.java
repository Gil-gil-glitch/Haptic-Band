...
public class MainApp_2 extends Application {
    ...
    private final Sphere topSphere = new Sphere(6);
    private final Sphere rightSphere = new Sphere(6);
    private final Sphere bottomSphere = new Sphere(6);
    private final Sphere leftSphere = new Sphere(6);

    private final PhongMaterial topMat = new PhongMaterial(Color.DARKRED);
    private final PhongMaterial rightMat = new PhongMaterial(Color.DARKRED);
    private final PhongMaterial bottomMat = new PhongMaterial(Color.DARKRED);
    private final PhongMaterial leftMat = new PhongMaterial(Color.DARKRED);
    ...
    private Node build3DView() {
        Cylinder wrist = new Cylinder(40, 100);
        wrist.setMaterial(new PhongMaterial(Color.web("#555555")));
        ...
        // Map motors to the cardinal points around the cylinder
        topSphere.setTranslateZ(-40);
        bottomSphere.setTranslateZ(40);
        rightSphere.setTranslateX(40);
        leftSphere.setTranslateX(-40);

        Group wristGroup = new Group(wrist, topSphere, rightSphere, bottomSphere, leftSphere);

        // Lay the cylinder horizontally (representing an arm extending right) and apply IMU rotations
        wristGroup.getTransforms().addAll(new Rotate(90, Rotate.Z_AXIS), rotateX, rotateY, rotateZ);
        ...
        SubScene subScene = new SubScene(wristGroup, 400, 200, true, SceneAntialiasing.BALANCED);
        subScene.setFill(Color.web("#282b30"));
    ...
    private Node buildMockIMU() {
        ...
        pitchSlider.setPrefWidth(280);
        yawSlider.setPrefWidth(280);
        rollSlider.setPrefWidth(280);

        // Bind UI sliders directly to the 3D rotation transforms
        ...
        grid.addRow(0, new Label("Pitch (X)"), pitchSlider);
        grid.addRow(1, new Label("Yaw (Y)"), yawSlider);
        grid.addRow(2, new Label("Roll (Z)"), rollSlider);
    ...
    private Node buildCenter() {
        ...
        // Injected the 3D view and Mock IMU right above the 2D cross
        center.getChildren().addAll(build3DView(), buildMockIMU(), crossPane, buildManualControls(), buildPresets());
        return center;
    }
    ...
    private void applyAndSend(int top, int right, int bottom, int left, String source) {
        ...
        // Interpolate sphere colors from dark red to bright red based on PWM
        topMat.setDiffuseColor(Color.web("#440000").interpolate(Color.RED, top / 255.0));
        rightMat.setDiffuseColor(Color.web("#440000").interpolate(Color.RED, right / 255.0));
        bottomMat.setDiffuseColor(Color.web("#440000").interpolate(Color.RED, bottom / 255.0));
        leftMat.setDiffuseColor(Color.web("#440000").interpolate(Color.RED, left / 255.0));
